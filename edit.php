...
?>
 <div class="login">
      ...
      <form method="post" action="action.php?action=updateUser">
        <p><label>Имя пользователя</label><input type="text"     name="username" value="<?php print($username); ?>" placeholder="Username "></p>
        <p><label>Пароль</label><input type="text"     name="password" value="<?php print($password); ?>" placeholder="Password"></p>
        <p><label>Телефон</label><input type="text"        name="phone"    value="<?php print($phone); ?>"   placeholder="Password"></p>		
	<p><label>Пол</label>
        <select name='gender'>
